...
const SERVER_SCRIPT = require('path').join(CWD, 'dist', 'index.js');
...
describe('mcpServer CLI initialize e2e', () => {
    ...
    test('responds to initialize and exits on Ctrl+C', async () => {
        ...
        try {
            ...
            runner.start({ command: process.execPath, args: [SERVER_SCRIPT, 'stdio'], cwd: CWD, env: childEnv1 }, 5000);
    ...
    test('does not exit on its own for 5s when started without exit timeout', async () => {
        ...
        try {
            ...
            runner.start({ command: process.execPath, args: [SERVER_SCRIPT, 'stdio'], cwd: CWD, env: childEnv2 });
    ...
    test('auto-exits when start called with 5000ms exit timeout', async () => {
        ...
        try {
            ...
            runner.start({ command: process.execPath, args: [SERVER_SCRIPT, 'stdio'], cwd: CWD, env: childEnv3 }, 5000);
